        ...
        <section class="mt-12 relative">
            ...
            <div class="flex justify-center">
               <table class="w-4/5 rounded-lg">
                    ...
                    <tr v-for="(item, index) in getData.items" :key="index" :class="index % 2===0 ? 'bg-green-200' : 'bg-white' ">
                        ...
                        <td class="flex justify-center content-center mt-6" v-if="(item.list_for == list_for)">
                            ...
                            <button @click="showEdit.showedit = true">
                                <svg @click="updateItem(item.id)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4 ml-2 cursor-pointer text-green-500 hover:size-6 duration-500 ">
            ...
            <section v-if="showEdit.showedit" class="">
                <section class=" top-0 left-60 flex justify-center border-y-green-300 m-y-12 border-2 rounded-lg bg-gray-100 bg-opacity-100 ">
                       <div class="">
                        <edit-items/>
                       </div> 
                </section>
...
export default{
    ...
    setup() {
        ...
        // function to get the items to be updated
        const updateItem = (item_id) => {
            axios.get(`/api/editItems/${item_id}`)
            .then(response => {
                getData.editItems = response.data
                if (response == 200){
                    console.log('update')
                }
            })
